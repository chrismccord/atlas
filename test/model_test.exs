...
defmodule Atlas.Fixtures.User do
  ...
  def lives_in_ohio(user) do
    unless user.state == "OH" do
...
defmodule Atlas.UserTest do
  ...
  test "#validate returns {:ok, record} when all validations return no errors" do
    assert SimpleUser.validate(SimpleUser.new(name: "Chris")) ==
      { :ok, SimpleUser.new(name: "Chris") }
  end

  test "#validate returns {:error, reasons} when validations return errors" do
    record = SimpleUser.new(name: "Name Too Long")
  ...
  test "#validate returns {:ok, record} if all validations pass" do
    {:ok, user} = SimpleUser.validate(SimpleUser.new(name: "Chris"))
    assert user.name == "Chris"
  end

  test "validate returns {:ok, record, errors} is any validation fails" do
    {:error, _, errors} = SimpleUser.validate(SimpleUser.new(name: nil))
    assert errors == SimpleUser.errors(SimpleUser.new(name: nil))

    {:error, _, errors} = SimpleUser.validate(SimpleUser.new(name: "A"))
    assert errors == SimpleUser.errors(SimpleUser.new(name: "A"))

    {:error, _, errors} = SimpleUser.validate(SimpleUser.new(name: "Name Too Long"))
    assert errors == SimpleUser.errors(SimpleUser.new(name: "Name Too Long"))
  end

  test "#errors returns array of attribute, error message tuples" do
    assert List.first(SimpleUser.errors(SimpleUser.new(name: "Name Too Long"))) == {
  ...
  test "#validates_length_of with greather_than" do
    ...
    refute User.errors_on(User.new(name: "Ted"), :name)
           |> Enum.member?("name must be greater than or equal to 3 characters")
  end

  test "#validates_length_of with greather_than and less_than" do
    assert User.errors_on(User.new(name: "DJ DUBS TOO LONG"), :name)
           |> Enum.member?("name must be greater than 2 and less than 10 characters")
    refute User.errors_on(User.new(name: "DJ DUBS"), :name)
           |> Enum.member?("name must be greater than 2 and less than 10 characters")
  end
  ...
  test "#validates_length_of with greather_than and less_than_or_equal" do
    ...
    assert User.errors_on(User.new(name: "AB"), :name)
           |> Enum.member?("name must be greater than 2 and less than or equal to 10 characters")
    refute User.errors_on(User.new(name: "DJ DUBS"), :name)
           |> Enum.member?("name must be greater than 2 and less than or equal to 10 characters")
  end
  ...
  test "#validates_presence_of" do
    assert User.errors_on(User.new(name: nil), :name)
           |> Enum.member?("name must not be blank")
    refute User.errors_on(User.new(name: "Max"), :name)
           |> Enum.member?("name must be greater than 2 and less than 10 characters")
  end

  test "#validates_numericality_of" do
    assert User.errors_on(User.new(total: "bogus"), :total)
           |> Enum.member?("total must be a valid number")
    refute User.errors_on(User.new(total: "1234"), :total)
           |> Enum.member?("total must be a valid number")
    refute User.errors_on(User.new(total: "-12.34"), :total)
           |> Enum.member?("total must be a valid number")
    assert User.errors_on(User.new(total: ""), :total)
           |> Enum.member?("total must be a valid number")
    refute User.errors_on(User.new(total: 1234), :total)
           |> Enum.member?("total must be a valid number")
    refute User.errors_on(User.new(total: -12.34), :total)
           |> Enum.member?("total must be a valid number")
    assert User.errors_on(User.new(total: nil), :total)
           |> Enum.member?("total must be a valid number")
    assert User.errors_on(User.new(total: []), :total)
           |> Enum.member?("total must be a valid number")
    assert User.errors_on(User.new(total: true), :total)
           |> Enum.member?("total must be a valid number")
  end


  test "#validates_numericality_of with greather_than" do
    assert User.errors_on(User.new(total: 2), :total)
           |> Enum.member?("total must be greater than 2")
    refute User.errors_on(User.new(total: 3), :total)
           |> Enum.member?("total must be greater than 2")
  end
  ...
  test "#validates_numericality_of with greather_than_or_equal" do
    ...
    refute User.errors_on(User.new(total: 10), :total)
           |> Enum.member?("total must be greater than or equal to 3")
  end

  test "#validates_numericality_of with greather_than and less_than" do
    assert User.errors_on(User.new(total: 19), :total)
           |> Enum.member?("total must be greater than 20 and less than 100")
    assert User.errors_on(User.new(total: 101), :total)
           |> Enum.member?("total must be greater than 20 and less than 100")
    refute User.errors_on(User.new(total: 99), :total)
           |> Enum.member?("total must be greater than 20 and less than 100")
  end

  test "#validates_numericality_of with greather_than and less_than_or_equal" do
    assert User.errors_on(User.new(total: 50), :total)
           |> Enum.member?("total must be greater than 50 and less than or equal to 80")
    assert User.errors_on(User.new(total: 81), :total)
           |> Enum.member?("total must be greater than 50 and less than or equal to 80")
    refute User.errors_on(User.new(total: 60), :total)
           |> Enum.member?("total must be greater than 50 and less than or equal to 80")
  end

  test "#validates_format_of" do
    assert User.errors_on(User.new(name: "Chris"), :name)
           |> Enum.member?("name is not valid")
    refute User.errors_on(User.new(name: "Chris McCord"), :name)
           |> Enum.member?("name is not valid")
  end
  ...
  test "#validates_inclusion_of" do
    ...
    refute User.errors_on(User.new(name: "jane"), :name)
           |> Enum.member?("name must be one of jane, bob")
  end
  ...
  test "#validates allows adding custom validations" do
    assert User.errors_on(User.new(state: "CA"), :state)
           |> Enum.member?("You must live in Ohio")
    refute User.errors_on(User.new(state: "OH"), :state)
           |> Enum.member?("You must live in Ohio")
  end
end
